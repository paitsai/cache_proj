...
sleep 10
python evaluate_qwen.py --cache_type xzr --save_dir xzr_results >> ../$LOG_FILE 2>&1

# 测试windows缓存类型
echo "测试windows缓存类型，等待10秒..." | tee -a ../$LOG_FILE
sleep 10
python evaluate_qwen.py --cache_type windows --save_dir windows_results >> ../$LOG_FILE 2>&1

# 测试full缓存类型
echo "测试full缓存类型，等待10秒..." | tee -a ../$LOG_FILE
sleep 10
python evaluate_qwen.py --cache_type full --save_dir full_results >> ../$LOG_FILE 2>&1
